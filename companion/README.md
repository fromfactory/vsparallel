...
heartbeat for the current VS Code or Antigravity IDE window to the VSParallel
desktop app. It reports workspace names, local folder paths, an ephemeral
...
or `antigravity_ide`. The companion derives this classification from the public
`vscode.env.uriScheme` field, with `vscode.env.appName` as a compatibility
...
`VSPARALLEL_ANTIGRAVITY_IDE_PROFILE`; status, installation, verification, and
